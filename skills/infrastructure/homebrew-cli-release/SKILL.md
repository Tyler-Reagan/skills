...
2. **Inject version into the binary** *(build-tool-specific)* — add a build-time-overridable version var + `--version`/`--help` flags. Go: linker `-ldflags -X`. Rust: `env!("CARGO_PKG_VERSION")` or a build script. Verify a manual build prints the injected value before wiring CI. (REFERENCE.md §Version)
...
5. **Add the release workflow + token** — `.github/workflows/release.yml` runs GoReleaser on `v*` tags. Create a **cross-repo PAT** (Contents: read+write on the *tap* repo only) and store it as the `HOMEBREW_TAP_GITHUB_TOKEN` repo secret. (REFERENCE.md §Workflow, §Token)
...
- **The default `GITHUB_TOKEN` cannot push to the tap.** It only has write on the *current* repo. The cask push to a *different* repo (`homebrew-tap`) needs a PAT. Symptom if missing/mis-scoped: release builds fine, then 403/404 at the cask step.
